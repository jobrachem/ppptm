...
class TransformationSpline:
    ...
    def dot_and_deriv(self, x: Array, coef: Array) -> tuple[Array, Array]:
        ...
        dist: batch: (n,); event: (loc,)

        """
        dd = jax.vmap(self.dot_and_deriv_n_fullbatch, (1, 0))
        dot, deriv = dd(x, coef)
        return dot.T, deriv.T
        # x = jnp.expand_dims(x, -1)
        # coef = jnp.expand_dims(coef, 0)
        # dot, deriv = self._dot_and_deriv(x, coef)
        # return jnp.squeeze(dot, -1), jnp.squeeze(deriv, -1)
    ...
    def dot_inverse(self, x: Array, coef: Array) -> Array:
        ...
        shape (B,P) or (B, n, P).
        """
        dd = jax.vmap(self.dot_inverse_n, (1, 0))
        return dd(x, coef).T
        # return dot.T, deriv.T
        # x = jnp.expand_dims(x, -1)
        # coef = jnp.expand_dims(coef, 0)
        # return jnp.squeeze(self._dot_inverse(x, coef), -1)
